...
class Params
{
  public:
    ...
    const float min_x_range = -50.00;
    const float max_x_range = 50.00;
    const float min_y_range = -40.00;
    const float max_y_range = 40.00;
    const float min_z_range = -0.5;
    ...
    const float anchor_bottom_heights[num_classes] = {-1,-0.66,-0.33,};
    // the score threshold for classification
    const float score_thresh = 0.55;
    const float nms_thresh = 0.55;
